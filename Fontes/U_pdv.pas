...
type
  TF_PDV = class(TForm)
    ...
    SQL_listar_pedidos_dbglançamentocli_id: TIntegerField;
    SQL_listar_pedidos_dbglançamentocli_nome: TStringField;
    SQL_listar_pedidos_dbglançamentocli_endereco: TStringField;
    SQL_listar_pedidos_dbglançamentocli_numero: TStringField;
    SQL_listar_pedidos_dbglançamentocli_bairro: TStringField;
    SQL_listar_pedidos_dbglançamentocli_cidade: TStringField;
    SQL_listar_pedidos_dbglançamentocli_fone: TStringField;
    SQL_listar_pedidos_dbglançamentocli_celular: TStringField;
    SQL_listar_pedidos_dbglançamentocli_rg: TStringField;
    SQL_listar_pedidos_dbglançamentocli_cnpj_cpf: TStringField;
    SQL_listar_pedidos_dbglançamentocli_profissao: TStringField;
    SQL_listar_pedidos_dbglançamentocli_data_nasc: TDateField;
    SQL_listar_pedidos_dbglançamentoforma_id: TIntegerField;
    SQL_listar_pedidos_dbglançamentoforma_nome: TStringField;
    SQL_listar_pedidos_dbglançamentouser_id: TIntegerField;
...
begin
  ...
  //ProcedureAtualizaDBGridLançamentos;
  if SQL_listar_pedidos_dbglançamento.RecordCount > 0 then
  begin
    ShowMessage('Existem itens lançados! Para sair: Finalize ou Cancele a Venda, ou Remova o Item.');
    Action := caNone;
    Exit;
  end;
  codigo_venda := '';
...
begin
  ...
  if SQL_listar_pedidos_dbglançamento.IsEmpty then
  begin
    ShowMessage('Não existem produtos lançados!');
    edt_pro_nome_pdv.SetFocus;
    Exit;
  end;
  with SQL_itens_add do
...
begin
  ...
  begin
   ...
   if SQL_listar_pedidos_dbglançamentopro_estoque.Value < qtd then
  begin
...
begin
  ...
  if SQL_listar_pedidos_dbglançamentopro_estoque.Value >= 1 then
  begin
  with SQL_itens_add do
  begin
    //remove a quantidade da do campo QTD
...
begin
  ...
  //atualizo o estoque
  SQL_listar_pedidos_dbglançamento.First;
  while not SQL_listar_pedidos_dbglançamento.eof do
  begin
  with SQL_estoque do
  begin
    Close;
    ...
    SQL.Add('update produtos set pro_estoque = pro_estoque + :qtd');
    SQL.Add('where pro_id = :produto');
    ParamByName('qtd').Value := SQL_listar_pedidos_dbglançamentoiten_qtd.Value;
    ParamByName('produto').Value := SQL_listar_pedidos_dbglançamentopro_id.Value;
    ExecSQL;
  end;
  SQL_listar_pedidos_dbglançamento.Next;
